...
│   └── Never import core/* into shared/
├── shared/        # Reusable UI, models, pipes, directives, pages layouts
│   ├── components/   # Headless/stateless UI (rk-button etc.)
│   ├── pages/        # Reusable layout shells
│   ├── services/     # Utility services (NotificationService, etc.)
│   ├── models/       # Pure TS interfaces/types
│   ├── directives/
│   └── pipes/
└── features/      # Lazy-loaded business features
    ...
    │   ├── components/
    │   ├── pages/
    │   ├── services/   # Internal to the feature
    │   ├── models/
    │   └── {feature}.routes.ts
    └── A feature must never import internals from another feature.
```

Every folder above contains a `README.md` that describes its purpose and conventions. Use them to orient yourself when generating or moving files.
