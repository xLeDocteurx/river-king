...
| File | Responsibility |
| --- | --- |
| `src/styles/theme.scss` *(modify, comments only)* | One-line role comment above each of the 15 tokens in both theme blocks |
...
| `AGENTS.md` *(modify)* | Imperative "Design system (mandatory)" subsection at end of "Styling rules" |
...
- Produces: canonical recipes referenced by name from `interactions.md`, `checklist.md`,
  README, and future phase-2 plans: *primary button*, *outline button*, *ghost icon button*,
  *destructive icon button*, *dashed action row*, *clickable card*, *selectable list item*,
  *properties panel*, *input*, *dialog*, *empty state*.
