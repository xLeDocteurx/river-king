...
```html
class="tw-flex-1 tw-flex tw-flex-col tw-items-center tw-justify-center tw-bg-background tw-p-4
tw-gap-4"
```
...
it('should zoom in on wheel up and zoom out on wheel down', () => {
  ...
  canvas.dispatchEvent(
    new WheelEvent('wheel', { deltaY: -100, clientX: 128, clientY: 128, bubbles: true }),
  );
